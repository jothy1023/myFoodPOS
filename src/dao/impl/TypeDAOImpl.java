...
public class TypeDAOImpl implements TypeDAO {
	...
	public boolean createType(Type type) {
		try {
			...
			session.save(type);
			System.out.println("新增Type：" + type.getTname() + "成功! 编号："
					+ type.getTid());
			// 事务要在返回之前commit
			tx.commit();
			return true;
	...
	public Type getTypeByTid(int tid) {
		try {
			...
			// 判断是否查到food
			if (type != null) {
				System.out.println("查到Type：(" + type.getTid() + ") "
						+ type.getTname());
				return type;
			} else {
	...
	public boolean updateType(int tid, String tname) {
		try {
			...
			type.setTname(tname);
			session.update(type);
			System.out.println("修改Type Name：" + oldFName + "为"
					+ type.getTname() + "成功");
			// 事务要在返回之前commit
			tx.commit();
			return true;
	...
	public Type deleteTypeById(int tid) {
		try {
			...
			Type type = (Type) session.get(Type.class, tid);
			session.delete(type);
			System.out.println("删除Type：(" + type.getTid() + ") "
					+ type.getTname() + "成功");
			// 事务要在返回之前commit
			tx.commit();
			return type;
		...
		try {
			...
			if (listSize > 0) {
				...
				for (int i = 0; i < listSize; i++) {
					...
					System.out.println("(" + types[i].getTid() + ") "
							+ types[i].getTname());
				}
				// 事务要在返回之前commit
				tx.commit();
